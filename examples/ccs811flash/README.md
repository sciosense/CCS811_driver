# THIS PROCEDURE IS A COPY OF [github.com/maarten-pennings/ccs811](https://github.com/maarten-pennings/CCS811/tree/master/examples/ccs811flash)
...
Note that from version 2.0.0 ams/ScioSense has this change note "Removed NTC functionality. Pin 8 not measured and left undriven.".
...
As the [datasheet](http://sciosense.com/ccs811) explains the boot loader can be used to replace the gas application.
Since most Chinese boards come with firmware 1.1.0, and ams/ScioSense has released firmware 2.0.0 (around april 2018), Maarten Pennings has written 
this example: it flashes 2.0.0 into a CCS811.


## Firmware
The firmware was available from the [ams/Sciosense web](http://sciosense.com/ccs811).
Maarten Pennings has downloaded the "CCS811 Application firmware version 2.0.0" zip file.
...
As the readme in the zip explains, new fresh sensors should use firmware 2-0-0, and sensors that have run for a number of days should use firmware 2-0-1. As the revision history in the zip explains, the 2-0-1 has
all the features of the 2-0-0, except that the burn-in compensation is disabled (hence it should be used on sensors that are already burned in).
...
Maarten Pennings explained how he believe it works in more detail in an 
...
Maarten Pennings has included that [CCS811_FW_App_v1-0-0.bin](CCS811_FW_App_v1-0-0.bin) and the converted 
[CCS811_FW_App_v1-0-0.h](CCS811_FW_App_v1-0-0.h) header in this repo, in case you want to downgrade.

## Process
Maarten Pennings has downloaded the 2.0.x firmware from the ams/ScioSense pages.
Maarten Pennings has written a Python script [hex.py](hex.py), Python 3.x - not Python 2.x.
...
Finally, Maarten Pennings has written a sketch [ccs811flash.ino](ccs811flash.ino) that uses this array to flash the CSS811.
...
## Example
Maarten Pennings hooked CCS811 with 1.1.0 firmware to the ESP8266.
...
Then he pressed reset, and this was the output
```
Starting CCS811 flasher
...
## A brick?
Maarten Pennings once aborted flashing half way, which means you end up with a CCS811 without a valid app.
